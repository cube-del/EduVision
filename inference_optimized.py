...
# СТАЛО (быстро, занимает мало памяти)
    quantization_config = BitsAndBytesConfig(
        load_in_4bit=True,
        bnb_4bit_compute_dtype=torch.float16, # или bfloat16, если карта новая (RTX 30xx/40xx)
        bnb_4bit_quant_type="nf4"
    )

    model = AutoModel.from_pretrained(
        MODEL_PATH,
        quantization_config=quantization_config, # Добавляем конфиг
        low_cpu_mem_usage=True,
        use_flash_attn=False, # Можно попробовать True, если установлена flash-attn
        trust_remote_code=True,
        device_map="auto" # Позволит библиотеке самой распределить слои
    ).eval()
